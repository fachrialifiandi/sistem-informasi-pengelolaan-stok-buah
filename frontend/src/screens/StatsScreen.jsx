...
export default function StatsScreen({ navigation }) {
  ...
  useEffect(() => {
    setIsLoading(true);
    inventoryService.getStats().then((data) => {
      setStatsData(data);
      setIsLoading(false);
    }).catch(err => {
      console.error(err);
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 60 }} className="flex-1 px-5 pt-5">
          ...
          <View className="flex-row justify-between items-center mb-6 gap-2">
            <TouchableOpacity 
              onPress={() => Alert.alert("Pilih Periode", "Pilihan jangka waktu: Hari Ini, 7 Hari Terakhir, 30 Hari Terakhir.")}
              activeOpacity={0.7}
            ...
            >
              ...
              <Text className="text-xs font-bold text-gray-700 dark:text-gray-300">{period}</Text>
              <MaterialIcons name="expand-more" size={16} color="#6C7A71" />
            </TouchableOpacity>
